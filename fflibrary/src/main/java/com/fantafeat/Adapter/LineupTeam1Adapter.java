...
public class LineupTeam1Adapter extends RecyclerView.Adapter<LineupTeam1Adapter.ViewHolder> {
    ...
    public LineupTeam1Adapter(Context context, List<PlayerModel> playerModelList, String sportId, boolean isPoint, Team1PlayerClick listener, Fragment fragment) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new ViewHolder(LayoutInflater.from(context).inflate(R.layout.lineup_team1_item,parent,false));
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (TextUtils.isEmpty(model.getOther_text2())){
            if (isPoint)
                holder.team_player_point.setText(model.getPlayerType()+" • "+model.getPlayerAvgPoint()+" pts");//ALL • 156 pts
            else
                holder.team_player_point.setText(model.getPlayerType()+" • "+model.getPlayer_percent()+"%");//ALL • 156 pts
        }else {
     ...
     /*   if (isPoint)
        ...
        if (sportId.equalsIgnoreCase("1")){
            if(model.getTeamId().equals(matchModel.getTeam1())){
                CustomUtil.loadImageWithGlide(context,holder.team_player_img, MyApp.imageBase + MyApp.document,  model.getPlayerImage(),R.drawable.ic_team1_tshirt);
            }else {
                CustomUtil.loadImageWithGlide(context,holder.team_player_img, MyApp.imageBase + MyApp.document,  model.getPlayerImage(),R.drawable.ic_team2_tshirt);
            }
        ...
        holder.layClick.setOnClickListener(view -> {
            ...
            /*else if (sportId.equals(ConstantUtil.football_id)) {
                if (((LineupsSelectionFragment) fragment).validateFootballClick(model)) {
                    notifyItemChanged(holder.getAdapterPosition());
                }
            }else if (sportId.equals(ConstantUtil.baseball_id)) {
                if (((LineupsSelectionFragment) fragment).validateBaseballClick(model)) {
                    notifyItemChanged(holder.getAdapterPosition());
                }
            }else if (sportId.equals(ConstantUtil.basketball_id)) {
                if (((LineupsSelectionFragment) fragment).validateBasketballClick(model)) {
                    notifyItemChanged(holder.getAdapterPosition());
                }
            }else if (sportId.equals(ConstantUtil.kabaddi_id)) {
                if (((LineupsSelectionFragment) fragment).validateKabaddiClick(model)) {
                    notifyItemChanged(holder.getAdapterPosition());
                }
            }*/
            /*if (((LineupsSelectionFragment) fragment).validateClick(model)) {
                notifyItemChanged(holder.getAdapterPosition());
            }*/
