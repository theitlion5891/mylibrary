...
public class Team2AnnounceAdapter extends RecyclerView.Adapter<Team2AnnounceAdapter.ViewHolder>{

    private List<PlayerModel> playerModelList;
    private Context context;
    String sportId="";

    public Team2AnnounceAdapter(List<PlayerModel> playerModelList, Context context,String sportId) {
    ...
    @NotNull
    @Override
    public Team2AnnounceAdapter.ViewHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        return new ViewHolder(LayoutInflater.from(context).inflate(R.layout.team2_announce,parent,false));
    }
        ...
        if (!TextUtils.isEmpty(model.getPlayerImage())){
            if (sportId.equals("1")){
                CustomUtil.loadImageWithGlide(context,holder.imgPlayer,ApiManager.TEAM_IMG,model.getPlayerImage(),
                        R.drawable.ic_team2_tshirt);
            }
            else if (sportId.equals("2")){
                CustomUtil.loadImageWithGlide(context,holder.imgPlayer,ApiManager.TEAM_IMG,model.getPlayerImage(),
                        R.drawable.football_player2);
            }
            else if (sportId.equals("4")){
                CustomUtil.loadImageWithGlide(context,holder.imgPlayer,ApiManager.TEAM_IMG,model.getPlayerImage(),
                        R.drawable.basketball_team2);
            }
            ...
            if (sportId.equals("1")){
                holder.imgPlayer.setImageResource(R.drawable.ic_team2_tshirt);
            }
            else if (sportId.equals("2")){
                holder.imgPlayer.setImageResource(R.drawable.football_player2);
            }
            else if (sportId.equals("4")){
                holder.imgPlayer.setImageResource(R.drawable.basketball_team2);
            }
            else if (sportId.equals("3")){
                holder.imgPlayer.setImageResource(R.drawable.baseball_player2);
            }
            else if (sportId.equals("6")){
                holder.imgPlayer.setImageResource(R.drawable.handball_player2);
            }
            else if (sportId.equals("7")){
                holder.imgPlayer.setImageResource(R.drawable.kabaddi_player2);
            }
        }
