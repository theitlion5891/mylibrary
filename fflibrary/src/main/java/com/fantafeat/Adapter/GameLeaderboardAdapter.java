...
public class GameLeaderboardAdapter extends RecyclerView.Adapter<GameLeaderboardAdapter.ViewHolder> {
    ...
    public GameLeaderboardAdapter(Context context, List<GameLeaderboardModel> list, MyPreferences preferences, String winning_dec) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new ViewHolder(LayoutInflater.from(context).inflate(R.layout.user_leaderboard_item,parent,false));
    }

    @Override
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        GameLeaderboardModel item=list.get(position);

        if (winning_dec.equalsIgnoreCase("yes")){
            holder.txtWinAmt.setVisibility(View.VISIBLE);
            if (TextUtils.isEmpty(item.getTotalWinAmount())){
                holder.txtWinAmt.setVisibility(View.GONE);
            }else {
                if (item.getTotalWinAmount().equalsIgnoreCase("0")){
                    holder.txtWinAmt.setVisibility(View.GONE);
                }else {
                    if (item.getUserId().equals(preferences.getUserModel().getId())) {
                        holder.txtWinAmt.setText("You Won: "+ CustomUtil.displayAmount( context, item.getTotalWinAmount()));
                    }else {
                        holder.txtWinAmt.setText("Win: "+CustomUtil.displayAmount( context, item.getTotalWinAmount()));
                    }
                }
            }
        ...
        holder.txtTeamName.setText(item.getDisplayName());
        //holder.txtTeamName.setText(item.getDisplayName());
